# Anubis E2E Benchmark Harness (routed variant)
# Same as run_benchmark.ps1, but injects a workspace-level opencode.json that
# routes zai-coding-plan through the Anubis proxy (7878) with x-anubis-target
# pointing at the sleeve gateway (17321), which owns provider auth. Needed
# because the daemon runs in Direct mode without ~/.anubis/backups/ fallback.
#
# Usage: powershell -File harness/run_benchmark_routed.ps1 -TaskId task-001-rust-todo-cli
