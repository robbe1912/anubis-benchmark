# Anubis E2E Benchmark Harness
# Runs an AI agent (opencode) through the anubis proxy to build a real app.
# Captures all hallucination warnings, agent progress, and build results.
#
# Usage: powershell -File harness/run_benchmark.ps1 -TaskId task-001-rust-todo-cli
#
# Prerequisites:
#   - Anubis daemon running on 127.0.0.1:7878
#   - Opencode configured with zai-coding-plan provider
#   - oh-my-openagent plugin loaded (ralph_loop for autonomous continuation)
...
# --auto: auto-approve all permissions for autonomous operation
# --format json: structured JSON events to stdout
# -f: attach the task prompt as a file
# The ralph_loop in oh-my-openagent keeps the agent running autonomously
$OpencodeExe = "C:\Users\robin\.bun\bin\opencode.exe"
# IMPORTANT: opencode positional [message] must come BEFORE option flags
# -f is [array] so it consumes everything after it until next --flag
$OpencodeArgs = @(
    "run",
